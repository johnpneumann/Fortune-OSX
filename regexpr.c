...
#if 0
	/* re_search_pattern_2 has already done this, and otherwise we get little benefit
...
int re_match_pattern(bufp, string, size, pos, regs)
regexp_t bufp;
char *string;
int size, pos;
regexp_registers_t regs;
{
	return re_match_pattern_2(bufp, string, size, (char *)NULL, 0, pos, regs, size);
}


int re_search_pattern_2(bufp, string1, size1, string2, size2, pos, range, regs,
		mstop)
regexp_t bufp;
char *string1, *string2;
	...
	for (; range >= 0; range--, pos += dir)
	{
		if (fastmap)
		{
			if (dir == 1) /* searching forwards */
			{
				...
				if (translate)
					while (text != partend && !fastmap[(unsigned char)translate[(unsigned char)*text]])
						text++;
				else
			...
			{ /* searching backwards */
				...
				if (translate)
					while (text != partstart && !fastmap[(unsigned char)translate[(unsigned char)*text]])
						text--;
				else
		...
		{
			if (pos > 0 && (pos <= size1 ? string1[pos - 1] : string2[pos - size1 - 1]) != '\n')
				continue;
		}

		assert(pos >= 0 && pos <= size1 + size2);
		ret = re_match_pattern_2(bufp, string1, size1, string2, size2, pos, regs, mstop);
...
int re_search_pattern(bufp, string, size, startpos, range, regs)
regexp_t bufp;
char *string;
int size, startpos, range;
regexp_registers_t regs;
{
	return re_search_pattern_2(bufp, string, size, (char *)NULL, 0,
			   startpos, range, regs, size);
}

/* 94/01/19 (cjk) hide these on NeXT */
#if !defined(NeXT)
...
char *s;
{
	...
	return re_search_pattern(&re_comp_buf, s, len, 0, len, (regexp_registers_t)NULL) >= 0;
}
#endif /* !defined(NeXT) */
